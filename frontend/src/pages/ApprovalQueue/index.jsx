...
import { Shield, Check, X, Edit3, Search, Clock, Sparkles, Filter, CheckSquare, Mail, AlertCircle } from 'lucide-react';
...
const ApprovalQueue = () => {
  ...
  const handleApprove = async (id, customText = null) => {
    try {
      const result = await api.approveAIDraft(id, customText);
      setEditingId(null);
      setApprovalMessage({ type: 'success', text: result?.message || 'Draft Approved', detail: result?.detail || 'Email sending will be available in Sprint 6.' });
      ...
      {isConnected && (
        loading ? (
          ...
          <div className="space-y-4">
            {approvalMessage && (
              <div className={`p-4 rounded-xl flex items-center gap-3 text-sm ${approvalMessage.type === 'success' ? 'bg-green-50 text-green-800 border border-green-200 dark:bg-green-950/30 dark:text-green-300 dark:border-green-800' : 'bg-red-50 text-red-800 border border-red-200'}`}>
                <Check className="w-5 h-5 text-green-600 flex-shrink-0" />
                <div>
                  <span className="font-semibold">{approvalMessage.text}</span>
                  {approvalMessage.detail && <span className="ml-2 text-gray-600 dark:text-gray-400">{approvalMessage.detail}</span>}
                </div>
              </div>
            )}
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2 text-xs text-gray-400 flex-wrap">
                          ...
                          <span>•</span>
                          {item.prompt_template_name && <><span className="text-gray-300 dark:text-gray-600">📋 {item.prompt_template_name}</span><span>•</span></>}
                          {item.provider_name && <><span className="text-gray-300 dark:text-gray-600">🤖 {item.provider_name}</span><span>•</span></>}
                    ...
                    <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                      ...
                      {item.status === 'pending_review' ? 'Pending Review' : item.status === 'approved' ? '✓ Approved' : item.status === 'rejected' ? '✗ Rejected' : item.status}
                    </span>
                  </div>
                  ...
                  {item.email_body && (
                    <div className="bg-blue-50 dark:bg-blue-950/20 p-3 rounded-lg border border-blue-200 dark:border-blue-800">
                      <label className="text-[10px] font-bold text-blue-600 dark:text-blue-400 uppercase tracking-wider flex items-center gap-1 mb-1">
                        <Mail className="w-3 h-3" /> Original Email
                      </label>
                      <div className="text-[11px] text-gray-700 dark:text-gray-300 whitespace-pre-wrap max-h-24 overflow-y-auto">
                        {item.email_body}
                      </div>
                    </div>
                  )}
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between items-center">
                      <label className="text-[11px] font-bold text-gray-400 uppercase tracking-wider flex items-center gap-1">
                        <Sparkles className="w-3.5 h-3.5 text-indigo-500" /> Generated AI Draft
                      </label>
                  ...
                  {item.status === 'approved' && (
                    <div className="flex items-center gap-2 pt-2 border-t border-gray-100 dark:border-gray-700 text-xs text-green-600 dark:text-green-400">
                      <Check className="w-4 h-4" />
                      <span className="font-semibold">Draft Approved</span>
                      <span className="text-gray-400">— Email sending will be available in Sprint 6.</span>
                    </div>
                  )}
                </div>
              ))}
